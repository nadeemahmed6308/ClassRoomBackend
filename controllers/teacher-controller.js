...
//import Student from '../models/subjectSchema.js';

       
export const teacherRegister = async (req, res) => {
    const { name, email, password, role, school, teachSubject, teachClass } = req.body;
    try {
        const salt = await bcrypt.genSalt(10);
        const hashedPass = await bcrypt.hash(password, salt);

        const teacher = new Teacher({ name, email, password: hashedPass, role, school, teachSubject, teachClass });
    ...
    try {
        ...
        if (teacher) {
            ...
            if (validated) {
                ...
                teacher = await teacher.populate("teachClass", "className")
